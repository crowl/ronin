...
func TestFindSymbolDescriptionGuidesUse(t *testing.T) {
	...
	for _, expected := range []string{"exact file", "1-based line range", "targeted read_file", "before text search", "does not find references", "struct/interface fields", "use text search"} {
...
func TestOutlinePackageDescriptionGuidesUse(t *testing.T) {
	...
	for _, expected := range []string{"unfamiliar Go package", "exact files", "1-based line ranges", "pass their ranges to read_file", "find_symbol", "text search", "references", "implementations", "callers", "callees"} {
		if !strings.Contains(description, expected) {
			t.Errorf("description %q does not contain %q", description, expected)
		}
	}
