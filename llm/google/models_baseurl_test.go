...
func TestValidateBaseURL(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.baseURL, func(t *testing.T) {
			...
			if !tc.valid {
				if err == nil || !strings.Contains(err.Error(), "GEMINI_BASE_URL") {
					t.Fatalf("validateBaseURL() error = %v, want GEMINI_BASE_URL validation error", err)
				}
				return
			}
			if err != nil {
...
func TestNewLLMUsesDefaultBaseURL(t *testing.T) {
...
func TestSetupWithBaseURLRegistersModels(t *testing.T) {
	if os.Getenv("RONIN_GOOGLE_SETUP_WITH_BASE_URL") == "1" {
		const root = "https://proxy.example.com/v1beta"
		...
		for _, model := range models {
			...
			googleClient, ok := client.(*LLM)
			if !ok {
				t.Fatalf("client type = %T, want *LLM", client)
			}
			if googleClient.baseURL != root {
				t.Errorf("model %s base URL = %q, want %q", model, googleClient.baseURL, root)
			}
		}
		return
	}
	...
	cmd.Env = append(os.Environ(), "RONIN_GOOGLE_SETUP_WITH_BASE_URL=1")
...
func TestSetupWithBaseURLRoutesRequests(t *testing.T) {
	if os.Getenv("RONIN_GOOGLE_SETUP_WITH_BASE_URL_ROUTING") == "1" {
		var paths []string
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			paths = append(paths, r.URL.Path)
			if r.Header.Get("Accept") == "text/event-stream" {
				w.Header().Set("Content-Type", "text/event-stream")
				_, _ = w.Write([]byte("data: {\"event_type\":\"interaction.completed\"}\n\n"))
				return
			}
		...
		if err := SetupWithBaseURL("key", server.URL+"/v1beta///"); err != nil {
			t.Fatalf("SetupWithBaseURL() error = %v", err)
		}
		...
		if want := []string{"/v1beta/interactions", "/v1beta/interactions"}; !reflect.DeepEqual(paths, want) {
	...
	cmd.Env = append(os.Environ(), "RONIN_GOOGLE_SETUP_WITH_BASE_URL_ROUTING=1")
	if output, err := cmd.CombinedOutput(); err != nil {
